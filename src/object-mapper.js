...
export function ObjectMapper(src, dest, map) {
...
export function getKeyValue(src, keystr) {
...
function select(src, keys) {
  ...
  if (key.name) return select_obj(src, key, keys);
...
function select_obj(src, key, keys) {
  ...
  if (src && key.name) {
    ...
    if (key.name == "*") return select_obj_keys(src, keys);
...
export function setKeyValue(dest, keystr, data, context = {}) {
  ...
  if (Array.isArray(keystr)) {
    for (let i = 0; i < keystr.length; i++) {
      ...
      // The substring value is in object notation - dig further
        ...
        else dest = setKeyValue(dest, keystr[i].key, data, context);
      }
    }
  }
  ...
  // The value is in object notation - dig a bit further
    ...
    else dest = setKeyValue(dest, keystr.key, data, context);
  }
...
function update(dest, data, keys, context) {
  if (keys) {
    ...
    if (key.name) return update_obj(dest, key, data, keys, context);
...
function update_obj(dest, key, data, keys, context) {
  ...
  else dest = set_data(dest, key, data, context);
...
function update_arr_ix(dest, ix, data, keys, context) {
  ...
  else o = keys.length ? update(null, data, keys, context) : data;
...
//
export function parse(key_str, delimiter = ".") {
...
ObjectMapper.split = split;
ObjectMapper.parse = parse;
ObjectMapper.getKeyValue = getKeyValue;
ObjectMapper.setKeyValue = setKeyValue;
